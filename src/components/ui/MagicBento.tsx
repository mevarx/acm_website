...
const DEFAULT_GLOW_COLOR = '254, 188, 46'; // acm-yellow equivalent
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const processSpotlight = () => {
      ...
      gsap.to(spotlightRef.current, { x: e.clientX, y: e.clientY, duration: 0.1, ease: 'power2.out' });
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (spotlightRafId === null) {
        spotlightRafId = requestAnimationFrame(processSpotlight);
      }
    };
    ...
    return () => {
      if (spotlightRafId !== null) cancelAnimationFrame(spotlightRafId);
...
const BentoCardGrid = ({ children, gridRef, className }: { children: React.ReactNode; gridRef: React.RefObject<HTMLDivElement | null>; className?: string }) => (
...
const MagicBento = ({
  ...
  return (
    <>
      ...
      {/* We intercept the children to wrap them in the interactive MagicBento elements */}
